...
@export var recoil_position_z : Curve  # Backwards push
...
var recoil_recovery_delay: float = 0.1
var recovery_timer: float = 0.0

func _ready():
	# Store initial transform as base
	...
	if not recoil_rotation_x:
		...
		# Quick rise, slow fall for vertical recoil
	...
	if not recoil_rotation_z:
		...
		# Sideways recoil peaks slightly after vertical
...
func _physics_process(delta: float):
	...
	if is_recoiling:
		if current_time < 1.0:
			...
			target_rot.x = clamp(base_rotation.x + recoil_x, -max_recoil.x, max_recoil.x)
			target_rot.z = clamp(base_rotation.z + recoil_z, -max_recoil.y, max_recoil.y)
			target_pos.z = clamp(base_position.z + recoil_pos_z, -max_recoil.z, max_recoil.z)
			...
			if recovery_timer >= recoil_recovery_delay:
				...
				rotation = rotation.lerp(base_rotation, delta * return_speed)
				position = position.lerp(base_position, delta * return_speed)
				
				# Check if we're close enough to reset
				if rotation.distance_to(base_rotation) < 0.01 and position.distance_to(base_position) < 0.01:
					reset_targets()
...
func apply_recoil():
	...
	# Store current position as new base to prevent accumulation
	base_rotation = rotation
	base_position = position
